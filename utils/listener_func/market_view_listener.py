...
def parse_first_market_listing(
    ...
    for idx, line in enumerate(embed_description.splitlines()):
        ...
        if match:
            ...
            # Extract the full Pokédex emoji (e.g., <:383:821880023661019157>)
            pokemon_emoji_id = None
            all_emojis = re.findall(r"(<:[^:>]+:\d+>)", emoji_block)
            if len(all_emojis) > 1:
                pokemon_emoji_id = all_emojis[
                    1
                ]  # second emoji is usually Pokédex emoji
            else:
                pokemon_emoji_id = all_emojis[0] if all_emojis else None
            debug_log(f"pokemon_emoji_id: {pokemon_emoji_id}")

            return (full_name, price_each, date_listed, pokemon_emoji_id)
...
async def market_view_listener(bot: discord.Client, message: discord.Message):
    ...
    if listing:
        pokemon_name, price_each, date_listed, pokemon_emoji_id = listing
        debug_log(
        ...
        if emoji_id:
            ...
            if old_emoji_id != emoji_id:
                ...
                debug_log(
                    f"Emoji ID for {pokemon_name} is already up to date in cache. No update needed."
                )
